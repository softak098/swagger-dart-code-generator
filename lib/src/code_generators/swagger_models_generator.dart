...
abstract class SwaggerModelsGenerator extends SwaggerGeneratorBase {
  ...
  String generateModelClassContent(
    ...
    if (schema.type == 'array') {
      ...
      if (items != null) {
        ...
        if (options.overridenModels.firstWhereOrNull((e) => e.fileName == fileName)?.overridenModels.contains(getValidatedClassName(itemClassName)) ==
            true) {
          return '';
        }
  ...
  static String getValidatedParameterName(String parameterName) {
    ...
    final result =
        words.map((e) => e.split(RegExp(r'\W+|\_')).mapIndexed((int index, String str) => index == 0 ? str : str.capitalize).join()).join('\$');
...
'''
    ...
    final classExtra = validatedClassName.startsWith("MC") ? " implements IComboModel" : "";

    final generatedClass = '''
@JsonSerializable(explicitToJson: true $createToJson)
class $validatedClassName$classExtra{
...
@override
  ...
  String generateCopyWithContent(String generatedProperties, String validatedClassName) {
    ...
    final splittedCopyWithWrappedPropertiesNamesContent =
        splittedCopyWithWrappedProperties.map((e) => e.substring(e.indexOf(' ') + 1)).map((e) => '$e: ($e != null ? $e.value : this.$e)').join(',\n');
